# VMF-SNE implementation based on scikit-learn's TSNE implementation
...
def _joint_probabilities(distances, desired_perplexity, verbose, kappa):
    ...
    # Compute conditional probabilities using vMF distribution
    q_numerator = np.exp(kappa * cosine_similarities)
    conditional_P = q_numerator / np.sum(q_numerator, axis=1, keepdims=True)

    P = conditional_P + conditional_P.T

    # Ensure diagonal values are zero before using squareform
...
def _joint_probabilities_nn(distances, desired_perplexity, verbose, kappa):
...
def _kl_divergence(
    ...
    cosine_similarities = 1 - pdist(X_embedded, "cosine")
    q_numerator = np.exp(kappa * cosine_similarities)
    Q = np.maximum(q_numerator / (2.0 * np.sum(q_numerator)), MACHINE_EPSILON)
    ...
    PQd = squareform((P - Q) * cosine_similarities)
...
class TSNE(ClassNamePrefixFeaturesOutMixin, TransformerMixin, BaseEstimator):
    ...
    def _fit(self, X, skip_num_points=0):
        ...
        if self.method == "exact":
            ...
            P = _joint_probabilities(distances, self.perplexity, self.verbose, self.kappa)
            ...
            # compute the joint probability distribution for the input space
            P = _joint_probabilities_nn(distances_nn, self.perplexity, self.verbose, self.kappa)
        ...
        if isinstance(self.init, np.ndarray):
            ...
            X_embedded /= np.linalg.norm(X_embedded, axis=1, keepdims=True)
    ...
    def _tsne(
        ...
        opt_args = {
            ...
            "args": [P, degrees_of_freedom, n_samples, self.n_components, self.kappa],  # Add kappa
